...
[[ "$runner" == nmrpeak_chf_v1 ]] || fail "runner image must select nmrpeak_chf_v1: ${runner:-<unset>}"
...
[[ -d "/sys/class/net/$wifi_interface" ]] ||
    fail "cannot build the CHF runner because the Wi-Fi interface does not exist: $wifi_interface"
...
for _ in {1..100}; do
    [[ -s "$ready_file" ]] && break
    kill -0 "$proxy_pid" 2>/dev/null || fail "Wi-Fi proxy stopped before the CHF runner build."
    sleep 0.05
done
[[ -s "$ready_file" ]] || fail "Wi-Fi proxy did not become ready before the CHF runner build."
readonly proxy_url="http://127.0.0.1:$(<"$ready_file")"
readonly image="numpde/nmrpeak-chf-runner:${image_input_id#sha256:}"
...
docker build --network host \
    ...
    --file "$context/models/nmrpeak_chf_v1/runner/Dockerfile.runner" "$context"

readonly image_id="$(<"$image_id_file")"
[[ "$image_id" =~ ^sha256:[0-9a-f]{64}$ ]] || fail "Docker returned a malformed CHF runner image ID: $image_id"
[[ "$(docker image inspect "$image_id" --format '{{index .Config.Labels "org.opencontainers.image.revision"}}')" == "$revision" ]] ||
    fail "built CHF runner image does not record its committed source revision: $image_id"
[[ "$(docker image inspect "$image_id" --format '{{index .Config.Labels "io.numpde.nmrpeak.image.input-id"}}')" == "$image_input_id" ]] ||
    fail "built CHF runner image does not record its exact input identity: $image_id"
[[ "$(docker image inspect "$image_id" --format '{{.Config.User}}')" == 65532:65532 ]] ||
    fail "built CHF runner image does not use the fixed non-root identity: $image_id"
