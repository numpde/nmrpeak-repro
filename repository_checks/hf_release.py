"""HF identity over the shared safe checkpoint-release mechanics."""
...
SCHEMA_ID = "nmrpeak.checkpoint_release.hf.v1"
...
ARCHIVE_MEMBER = (
    ...
    "HF/checkpoint_best.pt"
)
HF_RELEASE_SPEC = CheckpointReleaseSpec(
    lane_name="HF",
    schema_id=SCHEMA_ID,
    runner_ref=RUNNER_REF,
    runner_contract_id=HF_RUNNER_CONTRACT_ID,
    analysis_kind_ref=HF_OFFERING.analysis_kind_ref,
    decode_policy_id=HF_RESULT_IDENTITY.decode_policy.decode_policy_id,
    archive_member=ARCHIVE_MEMBER,
)
HfReleaseRejected = CheckpointReleaseRejected
HfCheckpointRelease = CheckpointRelease
...
def measure_checkpoint_member(archive: Path) -> tuple[int, str]:
    return _measure_checkpoint_member(HF_RELEASE_SPEC, archive)


def main(argv: list[str] | None = None) -> int:
    return run_release_cli(HF_RELEASE_SPEC, argv)
